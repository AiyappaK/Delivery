...
const DeliveyPage = () => {
  ...
  setDisabled(true)
    ...
    const storageRef = ref(storage, `/delivery/${text}/${loggedInUser.name}/${compressedFile.name}`);
        ...
        getDownloadURL(fileRef.snapshot.ref).then((downloadUrl) => {
          ...
          if (typeOfPayment === "credit") {
           ...
           post(downloadUrl, c);
            ...
            let c = Number(UserBalance.current_balance) + Number(amount)
            post(downloadUrl, c);
          }
        });
      }
  ...
  let post = async (downloadUrl, balance) => {
    await addDoc(collection(db, "deliveryHistory"), {
      ...
      deliverdBy: loggedInUser.name,
      deliveredByUserId: loggedInUser.uid,
      downloadUrl,
      Cleared: false,
    }).then(async () => {
